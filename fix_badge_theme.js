...
// 1. Fix VerifiedAvatar.tsx (Avatar disappearing)
const avatarFile = 'starto-web/components/feed/VerifiedAvatar.tsx';
if (fs.existsSync(avatarFile)) {
    let content = fs.readFileSync(avatarFile, 'utf8');
    
    // Add crossOrigin attr to fix html2canvas CORS issue
    content = content.replace(
        '<img',
        '<img crossOrigin="anonymous"'
    );

    // Fix the text size from a percentage to a relative em value so it scales correctly, or explicitly use full width flex
    content = content.replace(
        'className="text-[45%] tracking-tighter uppercase font-bold"',
    ...
    console.log('Fixed VerifiedAvatar.tsx');
}

// 2. Redesign ShareBadge.tsx
...
`;
    ...
    fs.writeFileSync(badgeFile, newBadgeContent, 'utf8');
    console.log('Fixed ShareBadge.tsx theme mapping');
}
